...
import { ChatSDK, getChat, pickFromQueue, deactivateChat } from '../utils/livechat-chat-sdk.js';

const getLastMessages = async (chatInfo) => {
  try {
    const chatId = chatInfo?.id;
    const chatLastThreadId = chatInfo?.thread?.id || chatInfo?.last_thread_summary?.id;
  ...
  useEffect(() => {
    const handleThreads = ({ payload }) => {
      ...
      if (
        ...
        (payload?.chat?.threads?.length || payload?.chat?.thread)
      ) {
        const msgs = payload?.chat?.threads?.length
          ? payload?.chat?.threads[0]?.events
