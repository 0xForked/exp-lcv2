...
const ChatProvider = ({ children }) => {
  ...
  const getActiveChats = async () => {
    try {
      ...
      const activeChats = chats_summary.filter(
        (chat) =>
          chat?.last_thread_summary?.active &&
          !chat?.last_thread_summary?.queue &&
          chat.users[0].session_fields[0].nadiCustomerId === appointmentCustomerId
      );
  ...
  const getQueueChats = async () => {
    try {
      ...
      const queueChats = chats_summary.filter(
        (chat) => chat?.last_thread_summary?.active && chat?.last_thread_summary?.queue
      );
